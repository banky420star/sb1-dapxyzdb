...
app.post('/api/ai/consensus', async (req, res) => {
  try {
    ...
    const consensus = await getConsensusWithRisk(features);
    res.json(consensus);
  } catch (error) {
  ...
  try {
    ...
    tradingInterval = setInterval(async () => {
      try {
        ...
        // Get consensus
        const consensus = await getConsensusWithRisk(features, req.risk);
        ...
        if (consensus.passes) {
          ...
          await executeTrade({
            ...
            qty: consensus.riskParams.positionSize,
            sl: req.risk.sl,
            tp: req.risk.tp
          });
...
const server = app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Autonomous Trading Bot listening on http://0.0.0.0:${PORT}`);
